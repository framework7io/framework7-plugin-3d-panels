...
function es(cb) {
  ...
  rollup({
    input: './src/framework7.3dpanels.js',
    plugins: [
      replace({
        delimiters: ['', ''],
    ...
    .pipe(source('framework7.3dpanels.js', './src'))
    .pipe(buffer())
    .pipe(rename('framework7.3dpanels.esm.js'))
...
function umd(cb) {
  ...
  rollup({
    ...
    .on('end', () => {
      ...
      // Minified version
      gulp.src('./dist/framework7.3dpanels.js')
